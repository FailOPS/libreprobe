...
typedef struct {
  ...
  /*
    client <--------------> libreprobe <-----------------------------> server
    |<- clientNwLatency ->|        |<- serverNwLatency --------->|
    |<--------------- network delay/latency -------------------->|
  */
  struct timeval clientNwLatency; /* The RTT/2 between the client and libreprobe */
  struct timeval serverNwLatency; /* The RTT/2 between libreprobe and the server */
