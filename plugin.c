...
#define PLUGIN_DIR_SYSTEM PREFIX "/lib/libreprobe/plugins"
...
void loadPlugins() {
  ...
  if(directoryPointer == NULL) {
    traceEvent(TRACE_WARNING, "Unable to find plugins directory. libreprobe will work without plugins!");
  } else {
...
static void loadPlugin(char *dirName, char *pluginName) {
  ...
  if(pluginInfo != NULL) {
    if(strcmp(pluginInfo->libreprobe_revision, libreprobe_revision)) {
      traceEvent(TRACE_WARNING, "Plugin %s (%s/%s) version mismatch [loaded=%s][expected=%s]: %s",
		 pluginInfo->name, dirName, pluginName,
		 pluginInfo->libreprobe_revision, libreprobe_revision,
...
void printMetadata(FILE *file) {
  ...
  fprintf(file,
	  "#\n"
	  "# Generated by libreprobe v.%s (%s) for %s\n"
	  "# on %s"
	  "#\n",
