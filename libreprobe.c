...
static const struct option _long_options[] = {
  ...
  */
  ...
  { "libreprobe-version",                   required_argument, NULL, 252 /* dummy */ },
...
void cleanup(int signo) {
  ...
  if(!readOnlyGlobals.libreprobe_up) exit(0);
  ...
  readOnlyGlobals.libreprobe_up = 0;
		    ...
		    */
		    ...
		    if(first_fragment == 0) {
			 ...
			 than the first one */
		      ...
		      /* In this case we tell libreprobe not to consider as length the one
...
int isGoodLicense(char **rcmsg) {
  ...
  *rcmsg = print_license_code(rc);
  ...
  if(rc != 0) {
    ...
    snprintf(msg, sizeof(msg), "Invalid or missing libreprobe License file %s [%s]", license_path, *rcmsg);
...
void probeVersion(void) {
  ...
  printf("\nWelcome to libreprobe v.%s (r%d) for %s\n"
	 "%s\n"
  ...
  if(!isGoodLicense(&msg)) {
    printf("WARNING: Invalid libreprobe license ("
#ifndef WIN32
	   "/etc/"
#endif
	   LICENSE_FILE_NAME") [%s]\n", msg);
  } else {
    printf("Valid libreprobe license found\n");
  }
#endif
}
...
void usage(u_int8_t long_help) {
  ...
  printf("libreprobe -n <host:port|none> [-i <interface|dump file>] [-t <lifetime timeout>]\n"
  ...
  printf("[--libreprobe-version|-v]               | Prints the program version.\n");
  ...
  printf("[--max-num-flows|-M] <max num flows>| Limit the number of active flows. This is\n"
	 ...
	 "                                    | or CPU allocated to libreprobe in case of non\n"
  ...
  printf("[--sample-rate|-S] <pkt rate>:<flow rate>\n"
	 ...
	 "                                    | '@' it means that libreprobe will report\n"
  ...
  printf("--city-list <city list>             | GeoIP file containing the city/IP mapping.\n"
	 "                                    | Note that libreprobe will load the IPv6 file\n"
  ...
  printf("--disable-l7-protocol-guess         | When nDPI is enabled, in case a protocol is not recognized,\n"
	 "                                    | libreprobe guesses the protocol based on ports. This option disables\n"
  ...
  printf("--dont-reforge-timestamps           | Disable libreprobe to reforge timestamps with -i <pcap file> and \n"
  ...
  printf("--ucloud                            | Enable the libreprobe micro-cloud\n");
#endif
  ...
  printf("\nExample: libreprobe -T \"%s\"\n", DEFAULT_V9_IPV4_TEMPLATE);

  printf("\n");
  printf("libreprobe shut down\n");
...
static int parseOptions(int argc, char* argv[], u_int8_t reparse_options) {
  ...
  if((argc == 2) 
    ...
    readOnlyGlobals.argv[readOnlyGlobals.argc++] = strdup("libreprobe");
    ...
    if(reparse_options) {
      ...
      traceEvent(TRACE_WARNING, "Please use libreprobe <configuration file>");
      return(-1);
    }
  ...
  while((opt = getopt_long(readOnlyGlobals.argc, readOnlyGlobals.argv,
	...
	*/
    ...
    case 'I':
      {
	u_int len = strlen(optarg), max_len = sizeof(readOnlyGlobals.libreprobeId)-1;

	if(len >= max_len) len = max_len;
	strncpy(readOnlyGlobals.libreprobeId, optarg, len);
	readOnlyGlobals.libreprobeId[len] = '\0';
	...
	traceEvent(TRACE_ERROR, "libreprobe supports 5 (NetFlow 5), 9 (NetFlow 9) and 10 (IPFIX)");
	exit(0);
      }
      break;
  ...
  */
  ...
  traceEvent(TRACE_NORMAL, "Welcome to libreprobe v.%s (%s) for %s %s",
	     version, libreprobe_revision, osName,
#ifdef HAVE_PF_RING
	     "with native PF_RING acceleration"
#else
	     ""
...
#if defined(HAVE_LICENSE) || defined(WIN32)
  {
    ...
    traceEvent(TRACE_NORMAL, "libreprobe SystemId: %s", sysId);
    free(sysId);
  }
#endif

  if(((fd = fopen("libreprobe.license", "r")) != NULL)
     || ((fd = fopen("/etc/libreprobe.license", "r")) != NULL)) {
    char license[256] = { 0 }, *ret;

    ret = fgets(license, sizeof(license), fd);
    fclose(fd);

    traceEvent(TRACE_NORMAL, "libreprobe License:  %s", license);
  }
...
static void stopCaptureFlushAll(void) {
  ...
  traceEvent(TRACE_INFO, "libreprobe is shutting down...");
...
void shutdown_libreprobe(void) {
  ...
  traceEvent(TRACE_INFO, "libreprobe terminated.");
  dumpLogEvent(probe_stopped, severity_info, "libreprobe stopped");
...
static int openDevice(char ebuf[], int printErrors, char *pcapFilePath) {
  ...
  if(attachToNetFilter() < 0) {
	...
	if(readOnlyGlobals.pcapPtr == NULL)  {
	  ...
	  if((getuid () && geteuid ()) || setuid (0)) {
	    if(printErrors) {
	      traceEvent(TRACE_ERROR, "libreprobe opens the network interface "
			 "in promiscuous mode, ");
...
static void compileTemplates(u_int8_t reloadTemplate) {
  ...
  if(readOnlyGlobals.netFlowVersion == 5) {
    ...
    */
    ...
    /*
      ...
      but they are functional to libreprobe
    */
    readOnlyGlobals.templateBuffers[V4_TEMPLATE_INDEX].templatePlugin =
      compileTemplate(&readOnlyGlobals.stringTemplateV4,
		      readOnlyGlobals.templateBuffers[V4_TEMPLATE_INDEX].v9TemplateElementList,
  ...
  if(reloadTemplate) {
    ...
    traceEvent(TRACE_INFO, "libreprobe is now operational...");
  }
...
(int argc, char *argv[]) {
  ...
  */
  if(!readOnlyGlobals.demo_mode) {
    ...
    if((argc == 2) && (!strcmp(argv[1], "-v"))) {
      ...
      printf("%s\n", getSystemId());
      exit(0);
    } else if((argc == 2) && (!strcmp(argv[1], "--libreprobe-version"))) {
      printf("%s\n", version);
      exit(0);
...
#if 0
      if((num_instances = verify_application_instances("libreprobe", out_buf, sizeof(out_buf))) != 0) {
	traceEvent(TRACE_ERROR, "Too many libreprobe instances (%d) running", num_instances);
	/* exit(-1); */
	readOnlyGlobals.demo_mode = 1;
      }
#endif
    ...
    if(readOnlyGlobals.demo_mode) {
      ...
      traceEvent(TRACE_ERROR, "Invalid libreprobe license"
		 " ("
#ifndef WIN32
		 "/etc/"
#endif
      ...
      traceEvent(TRACE_ERROR, "**                                               **");
      traceEvent(TRACE_ERROR, "**  Create your libreprobe license at                **");
      traceEvent(TRACE_ERROR, "**       http://www.nmon.net/mklicense/          **");
      traceEvent(TRACE_ERROR, "**                                               **");
      traceEvent(TRACE_ERROR, "***************************************************");
    } else
      traceEvent(TRACE_NORMAL, "Valid libreprobe license found");
...
#else
  if((argc == 2) && (!strcmp(argv[1], "--libreprobe-version"))) {
    printf("%s\n", version);
    exit(0);
  }
#endif
  ...
  traceEvent(TRACE_NORMAL, "Welcome to libreprobe v.%s for %s", version, osName);
  printCopyrights();

#ifndef WIN32
  if(readOnlyGlobals.useSyslog)
    openlog(readOnlyGlobals.libreprobeId, LOG_PID ,LOG_DAEMON);
#endif
  ...
  */
  ...
  readOnlyGlobals.libreprobe_up = 1;
  ...
  if(readOnlyGlobals.zmq.endpoint != NULL) {
    ...
    /* If libreprobe will be daemon, the zmq socket must be initialize after that the daemon process will be finished. */
...
#ifndef WIN32
    readOnlyGlobals.libreprobePid = getpid();

    if(readOnlyGlobals.pidPath) {
      FILE *fd = fopen(readOnlyGlobals.pidPath, "w");
      if(fd != NULL) {
	fprintf(fd, "%lu\n", readOnlyGlobals.libreprobePid);
	fclose(fd);
      } else
  ...
  if((readOnlyGlobals.pcapPtr
    ...
    if(unlikely(readOnlyGlobals.enable_debug)) {
      traceEvent(TRACE_WARNING, "*****************************************");
      traceEvent(TRACE_WARNING, "** You're running libreprobe in DEBUG mode **");
      traceEvent(TRACE_WARNING, "*****************************************");
    }
  ...
  readWriteGlobals->libreprobeStarted = 1;
  traceEvent(TRACE_NORMAL, "libreprobe started successfully");
  dumpLogEvent(probe_started, severity_info, "libreprobe started");
  ...
  if(readOnlyGlobals.pcapFile) {
    ...
    while(readOnlyGlobals.libreprobe_up) {
      // sleep(5); break;
      ntop_sleep(1);
    }
  }
