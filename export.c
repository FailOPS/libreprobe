...
#define EXPIRE_YEAR   2005
    ...
    if(time(NULL) > mktime(&expireDate)) {
      traceEvent(TRACE_ERROR, "Sorry: this copy of libreprobe is expired.\n");
      exit(0);
    }
  }
#endif
  ...
  if(readOnlyGlobals.demo_mode) {
	...
	traceEvent(TRACE_ERROR, "* NOTE: You have reached the max demo %d flows export: no more exports *\n",
	       MAX_DEMO_FLOWS);
	traceEvent(TRACE_ERROR, "* NOTE: no additional flows will be exported by this libreprobe instance      *\n");
	traceEvent(TRACE_ERROR, "***************************************************************************\n\n");
	msg_shown = 1;
      }
...
static int sendFlowData(CollectorAddress *collector, char *buffer,
  ...
  */
  ...
  /*
    ...
    collectors might not be able to catch up with libreprobe
  */
...
void sendNetFlowV9V10(u_char lastFlow,
  ...
  */
  if(sendTemplate) {
	...
	Interim solution: we forget scope with IPFIX leaving it to a future libreprobe release
      */
