...
#define libreprobe_strdup(a)  _strdup(a)
#ifndef localtime
...
#include "dirent.h"
#else
#define libreprobe_strdup(a)  strdup(a)
#endif
...
#endif
extern int libreprobe_sleep(int secToSleep);
...
#define LICENSE_FILE_NAME  "libreprobe.license"

/*
  2^32 minus a large value so that we won't wrap for sure
  Note that libreprobe can work as sFlow collector and thus
...
extern char libreprobe_revision[];
...
typedef struct {
  ...
  PluginEntryPoint *templatePlugin; /*
				       ...
				       fields not part of the base libreprobe
				    */
} TemplateBufferInfo;
...
typedef struct {
  ...
  /* Status */
  u_int8_t libreprobe_up, dequeueBucketToExport_up, num_active_plugins;
  ...
  unsigned long libreprobePid; /* 0 on Windows */
...
typedef struct {
  ...
  u_int8_t shutdownInProgress:2, stopPacketCapture:1, libreprobeStarted:1;
...
extern void shutdown_libreprobe(void);
...
#ifdef WIN32
extern char* libreprobe_strdup(const char *str);
extern int libreprobe_inet_pton(int af, const char *src, void *dst);

#define strdup(a)       libreprobe_strdup(a) /* _strdup(a) */
#define stricmp(a,b)    _stricmp(a,b)
#define snprintf	_snprintf
#define inet_pton(a, b, c) libreprobe_inet_pton(a, b, c)
