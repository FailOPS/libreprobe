...
#
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# Run this to generate all the initial makefiles for libreprobe
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#
...
MAJOR_RELEASE="7"
MINOR_RELEASE="0"
AC_INIT="AC_INIT([libreprobe],[$MAJOR_RELEASE.$MINOR_RELEASE.$TODAY])"
...
echo ""
echo "Starting libreprobe automatic configuration system v$version"
echo ""
...
do
  ...
  -h | --help)
    cat <<EOF
This script should help you to configure 'libreprobe'
...
case "${libtoolversion}" in
  *1\.3\.[[45]]\-freebsd\-ports*)
    ...
    echo "*******************************************************************"
    echo "*"
    echo "*ERROR: libreprobe requires libtool version 1.4 or newer..."
    echo "*"
...
case "${automakeversion}" in
  *1\.[[0-5]]*)
    ...
    echo "******************************************************************"
    echo "*"
    echo "*ERROR: libreprobe requires automake version 1.6 or newer..."
    echo "*"
    echo "*>>>   Unable to proceed with your request, aborting!"
    echo "*"
    echo "*******************************************************************"
...
case "${autoconfversion}" in
  *2\.[[0-4]]*)
    ...
    echo "******************************************************************"
    echo "*"
    echo "*ERROR: libreprobe requires autoconf version 2.53 or newer..."
    echo "*"
    echo "*>>>   Unable to proceed with your request, aborting!"
    echo "*"
    echo "*******************************************************************"
...
NDPI_URL=https://github.com/FailOPS/libreDPI/
if test -d nDPI; then
...
if [ ".${config}" = ".y" ]; then
  ...
  echo "Run ./configure and then make to compile libreprobe"
fi
echo ""
