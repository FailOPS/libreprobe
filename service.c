...
#define SZSERVICENAME        "libreprobe"

  // displayed name of the service
#define SZSERVICEDISPLAYNAME "libreprobe for Win32"
...
void installService(char *service_name, int argc, char **argv)
{
  ...
  if ( schSCManager ) {
	...
	if( !ChangeServiceConfig2(
      ...
      snprintf(szDescr, sizeof(szDescr), "libreprobe v.%s - NetFlow/IPFIX Probe. http://www.ntop.org/",
	       version);
...
extern int libreprobe_main(int argc, char *argv[]);
...
void* invokelibreprobe(void* _szAppParameters) {
  DWORD dwNewArgc, i;
  ...
  /* Example: C:\ntop\libreprobe\Release\libreprobe.exe */
  ...
  libreprobe_main(dwNewArgc, lpszNewArgv);
...
VOID ServiceStart (DWORD dwArgc, LPTSTR *lpszArgv)
{
  ...
  // createThread(&libreprobeThread, invokelibreprobe, NULL);
  ...
  AddToMessageLog(TEXT("About to start libreprobe"));
  pthread_create(&libreprobeThread, NULL, invokelibreprobe, (void*)strdup(szAppParameters));
  AddToMessageLog(TEXT("libreprobe started"));

  // Wait for the stop event to be signalled.
  WaitForSingleObject(hServerStopEvent,INFINITE);
  AddToMessageLog(TEXT("libreprobe terminated"));
...
void main(int argc, char **argv)
{
 ...
 // pthread_win32_process_attach_np();
  ...
  if(!isWinNT()) {
    ...
    invokelibreprobe(szAppParameters);
    return;
  }
  thisIsAservice = 0;
  ...
  if(argc > 1){
    char *service_name = "libreprobe for Win32";
	...
	_tprintf(TEXT("/r requires the service name as parameter\n"));
      ...
      printf("/i <service name> [libreprobe options] - Install libreprobe as service\n");
      printf("/c [libreprobe options]                - Run libreprobe on a console\n");
      printf("/r <service name>                  - Deinstall the service\n\n");
      printf("Example:\n"
	     "Install libreprobe as a service: 'libreprobe /i my_libreprobe -i 0 -n 192.168.0.1:2055'\n"
	     "Remove the libreprobe service:   'libreprobe /r my_libreprobe'\n\n");
      printf("Notes:\n"
	     "1. Type 'libreprobe /c -h' to see all options\n"
	     ...
	     "   control panel item.\n"
	     "3. You can install the libreprobe service multiple times\n"
  ...
  printf("\nNOTE:\nUnder your version of Windows, libreprobe is started as a service.\n");
  printf("Please open the services control panel to start/stop libreprobe,\n");
  printf("or type libreprobe /h to see all the available options.\n");
